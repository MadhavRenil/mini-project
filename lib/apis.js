...
async function fetchSerpApiFlights({
  ...
  const mappedFlights = flights.slice(0, 50).map((option, index) => {
  ...
  }).filter((flight) => isPositiveAmount(flight.price));

  return mappedFlights.length
    ? mappedFlights.sort((a, b) => (a.price || 0) - (b.price || 0))
    : null;
}
...
async function fetchSerpApiHotels(destination, checkIn, checkOut, adults = 1) {
  ...
  const stays = properties.slice(0, 50).map((hotel, index) => {
    ...
    const nearbyPlaces = Array.isArray(hotel.nearby_places)
      ? hotel.nearby_places
        .map((place) => place?.name || place?.transportations?.[0]?.type || '')
        .filter(Boolean)
        .slice(0, 5)
      : [];
    ...
    return {
      ...
      address: Array.isArray(hotel.address) ? hotel.address.join(', ') : (hotel.address || null),
      description: hotel.description || null,
      type: hotel.type || hotel.property_type || null,
      review_count: Number(hotel.reviews || hotel.extracted_reviews || 0) || null,
      location_rating: Number(hotel.location_rating || 0) || null,
      nearby_places: nearbyPlaces,
      cancellation: hotel.free_cancellation ? 'Free cancellation' : null,
      payment: hotel.extracted_price ? 'Pay now' : null,
      source: 'serpapi',
      source_label: 'Google Hotels via SerpApi'
    };
...
async function fetchHotelsRealTime(destination, checkIn, checkOut, adults = 1) {
  ...
  try {
    ...
    const mappedHotels = hotels.slice(0, 50).map((hotel, index) => {
      ...
      return {
        ...
        hotel_class: Number(property?.accuratePropertyClass || property?.propertyClass || property?.qualityClass || 0) || null,
        review_count: Number(property?.reviewCount || 0) || null,
        review_word: property?.reviewScoreWord || null,
        city: property?.wishlistName || null,
        checkin_from: property?.checkin?.fromTime || null,
        checkin_until: property?.checkin?.untilTime || null,
        checkout_from: property?.checkout?.fromTime || null,
        checkout_until: property?.checkout?.untilTime || null,
        latitude: Number(property?.latitude || 0) || null,
        longitude: Number(property?.longitude || 0) || null,
        summary: label || null,
        ...
        source: 'rapidapi',
        source_label: 'Booking.com via RapidAPI'
      };
    }).filter((hotel) => isPositiveAmount(hotel.price));

    return mappedHotels.length ? mappedHotels : null;
  } catch (_) {
    return null;
  }
}
