...
function addHotelToOptions(options, hotelType, nights = 2) {
  const { getHotelOption } = require('./apis');
  const hotel = getHotelOption(hotelType, nights);
  return options.map(opt => ({
    ...opt,
    hotel: hotel,
...
function fullPipelineWithSteps(source, destination, travelDate, budget, preferenceType, numTravelers, transportChoice, hotelType, hotelNights, apiFlightOptions, userId, preferences) {
  ...
  const withTravelers = applyPreferenceScoring(options, preferenceType, budget, numTravelers);
  options = addHotelToOptions(withTravelers, hotelType || 'midrange', hotelNights || 2);
