 ...
 * Uses simulated data for demonstration; live data can be layered in from providers such as SerpApi.
 */
...
// Preference types: Adventure, Luxury, Family, Solo â€” used for AI scoring
...
function mergeApiFlights(engineOptions, apiFlights, numTravelers, routeDistanceKm = null) {
  ...
  const flightOptions = apiFlights.slice(0, 50).map((f, i) => {
    const totalApiPrice = f.price_total != null ? Number(f.price_total) : null;
    const perTravelerPrice = f.price_per_traveler != null
      ? Number(f.price_per_traveler)
      : totalApiPrice != null
        ? (Number(totalApiPrice) / num)
        : Number(f.price || 0);
    ...
    trip_type: f.trip_type || null,
    return_date: f.return_date || null,
    stop_count: f.stop_count != null ? Number(f.stop_count) : null,
    ...
    price_total: totalApiPrice != null ? totalApiPrice : (perTravelerPrice * num)
  });
  });
...
function addHotelToOptions(options, hotelType, nights = 2, selectedHotel = null) {
  ...
  const requestedNights = Math.max(1, parseInt(nights, 10) || 1);
  let hotel = getHotelOption(hotelType, requestedNights);

  if (selectedHotel && (selectedHotel.price_per_night != null || (Array.isArray(selectedHotel.hotels) && selectedHotel.hotels.length))) {
    const rawHotels = Array.isArray(selectedHotel.hotels) && selectedHotel.hotels.length
      ? selectedHotel.hotels
      : [selectedHotel];

    let remainingNights = requestedNights;
    const normalizedHotels = rawHotels.map((item) => {
      const assignedNights = Math.max(0, Math.min(
        remainingNights,
        parseInt(item.assigned_nights ?? item.total_nights ?? 1, 10) || 0
      ));
      remainingNights -= assignedNights;
      if (!assignedNights) return null;
      const nightly = Math.max(0, Number(item.price_per_night ?? item.price) || 0);
      return {
        id: item.id || null,
        type: item.type || item.category || hotelType || 'midrange',
        name: item.name || 'Selected Hotel',
        price_per_night: nightly,
        assigned_nights: assignedNights,
        total_nights: assignedNights,
        total_cost: Math.round(nightly * assignedNights * 100) / 100,
        rating: item.rating != null ? Number(item.rating) : null,
        simulated: !!item.simulated,
        source: item.source || null,
        distance_to_center_km: item.distance_to_center_km != null ? Number(item.distance_to_center_km) : null,
        distance_to_airport_km: item.distance_to_airport_km != null ? Number(item.distance_to_airport_km) : null,
    ...
    if (normalizedHotels.length) {
      const totalAssignedNights = normalizedHotels.reduce((sum, item) => sum + item.assigned_nights, 0);
      const totalHotelCost = normalizedHotels.reduce((sum, item) => sum + item.total_cost, 0);
      const primaryHotel = normalizedHotels[0];
      const gallery = [...new Set(normalizedHotels.flatMap((item) => item.images || []).concat(primaryHotel.image || []).filter(Boolean))];
      hotel = {
        id: normalizedHotels.length === 1 ? primaryHotel.id || null : null,
        type: normalizedHotels.length === 1 ? primaryHotel.type : (hotelType || primaryHotel.type || 'midrange'),
        name: normalizedHotels.length === 1 ? primaryHotel.name : `${normalizedHotels.length} hotels selected`,
        primary_name: primaryHotel.name || 'Selected Hotel',
        stay_mode: normalizedHotels.length > 1 ? 'split' : 'single',
        price_per_night: totalAssignedNights ? Math.round((totalHotelCost / totalAssignedNights) * 100) / 100 : 0,
        total_nights: totalAssignedNights,
        total_cost: Math.round(totalHotelCost * 100) / 100,
        rating: normalizedHotels.length === 1 && primaryHotel.rating != null ? Number(primaryHotel.rating) : null,
        simulated: normalizedHotels.every((item) => !!item.simulated),
        source: [...new Set(normalizedHotels.map((item) => item.source).filter(Boolean))].join(',') || null,
        image: gallery[0] || primaryHotel.image || null,
        images: gallery,
